...
1. ~~Ручное тестирование редактора, багфиксы~~ ✔
2. ~~Идл-системы (тики/оффлайн-прогресс), интерполяция переменных~~ ✔
3. ~~Загрузка изображений как data-URI~~ ✔
4. Контент: сценарий интро по Тому III (капсула → лаборатория → выход в мир) — диалоги,
   сцены, использование условий/репутации. Самая ценная следующая задача.
5. RPG-глубина по мере надобности: инвентарь? прокачка? (обсудить с владельцем, что нужно игре).
6. Мобильная адаптация редактора не нужна — мобильные только для ИГРЫ.
7. Git-репозиторий не инициализирован — предложить пользователю.
