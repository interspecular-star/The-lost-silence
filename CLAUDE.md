...
«Материалы» блоков (2026-07-09, трек владельца — spatial UI): `src/runtime/boxfx.ts` —
поверхность spatial (стекло/скругление/рамка/тень) + анимированные рамки: Перелив/
Комета/Разряд/Скан/Дыхание (кольцо-маска `.bfx-ring`; Разряд = SVG feTurbulence).
`Theme.dialogueBoxStyle` (surface/border/glass/radius), редактор: «Материал диалогового
блока» в панели «Мир». **Лаборатория стилей `/style-lab.html`** — песочница на движке
(src/lab/main.ts), проект владельца не трогает; dev-only, в build не входит.
Этапы G2–G5 (варианты, кнопки, фракции, полиш) — в roadmap, блок G.
