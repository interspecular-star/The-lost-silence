...
1. NPC и отношения (редактор персонажей, формула репутации, HUD Осколка) — СЛЕДУЮЩАЯ.
2. Характеристики + предметы + инвентарь (drag-and-drop, манекен).
3. Бои (редактор мобов, QTE-сцена).
